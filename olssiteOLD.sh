...
function uninstall_ols_debian
{
    ...
    if [ $? = 0 ] ; then
        ...
        apt-get -y --purge remove lsphp$LSPHPVER lsphp$LSPHPVER-imap 'lsphp*'
        if [ $? != 0 ] ; then
...
111111111111function install_site
{
    if [ ! -e "$SITEPATH" ] ; then 
...
function usage
{
    ...
    echoG "./ols1clk.sh -s yourdomain.com     " "To install OpenLiteSpeed with yourdomain.com"."
    echo
...
if [ "x$INSTALLSITE" = "x1" ] ; then
    echoG "Please access http://localhost:$SITEPORT/ to finish setting up your Site."
