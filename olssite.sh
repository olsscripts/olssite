...
{
    echo
    echoY "Installing OpenLiteSpeed"
    ...
    if [ $? != 0 ] ; then
		...
		echo "[OK] OpenLiteSpeed Installed"
    fi
}
...
function install_ssl
{
        ...
        /usr/bin/certbot-auto certonly --standalone -n --preferred-challenges http --agree-tos --email $EMAIL -d $DOMAIN
        echo
...
function config_ols_admin
{
    if [ -e "$SERVER_ROOT/conf/httpd_config.conf" ] ; then
	    sed -i -e "s/\$SERVER_ROOT\/admin\/conf/\webadmin.key/\etc\/letsencrypt\/live\/$DOMAIN\/privkey.pem/g" -i.bkp "$SERVER_ROOT/admin/conf/admin_config.conf"
		sed -i -e "s/\$SERVER_ROOT\/admin\/conf/\webadmin.crt/\etc\/letsencrypt\/live\/$DOMAIN\/cert.pem/g" "$SERVER_ROOT/admin/conf/admin_config.conf"
	fi	
}

function change_ols_admin_port
{
...
{
    if [ -e "$SERVER_ROOT/conf/httpd_config.conf" ] ; then
        sed -i -e "s/adminEmails/adminEmails $EMAIL\" "$SERVER_ROOT/conf/httpd_config.conf"
...
function usage
{
    ...
    echoG " --domain(-d) DOMAIN               " "To install and setup your site with your chosen domain."
    echoG " --sitepath (-s) SITEPATH          " "To specify a location for the new site installation or use an existing site installation."
...
{
    test_page https://localhost:$ADMINPORT/ "LiteSpeed WebAdmin" "Load WebAdmin Page" 
}

function test_ols
{
    test_page http://localhost:$SITEPORT/  Congratulation "Load Example HTTP vhost Page" 
    test_page https://localhost:$SSLSITEPORT/  Congratulation "Load Example HTTPS vhost Page" 
}

function test_site
{ 
    test_page http://$DOMAIN:$SITEPORT/ "Congratulation" "Load HTTP first Page" 
    test_page https://$DOMAIN:$SSLSITEPORT/ "Congratulation" "Load HTTPS first Page" 
}
...
while [ "$1" != "" ] ; do
    case $1 in
        ...
        -d | --domain )             check_value_follow "$2" "domain"
                                    shift
                                    DOMAIN=$FOLLOWPARAM
		                            INSTALLSITE=1
                                    ;;
        ...
        -s | --sitepath )           check_value_follow "$2" "Site path"
                                    shift
...
if [ "x$SITEINSTALLED" != "x1" ] ; then
	...
	config_ols_site
	config_ols_admin
	change_ols_admin_port
	install_ssl
else
    #normal ols installation without a 
	gen_selfsigned_cert
    config_ols
	change_ols_admin_port
fi
