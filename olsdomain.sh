...
function install_ssl
{
        #SSL INSTALL#
        systemctl stop lsws
        /usr/bin/certbot-auto certonly --standalone -n --preferred-challenges http --agree-tos --expand --email $EMAIL -d $SITEDOMAIN,$VIRTHOST
...
while [ "$1" != "" ] ; do
    case $1 in     
        ...
        -s | --sitedomain )         check_value_follow "$2" "Site domain"
                                    shift
